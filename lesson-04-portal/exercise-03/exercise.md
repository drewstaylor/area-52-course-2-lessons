...
We want to know if a specific user has any passports. The `Tokens` query entry point ([read more](https://docs.rs/cw721/latest/cw721/enum.Cw721QueryMsg.html)) of cw721 tokens accepts an `owner` address as a parameter, so we'll use that for our check.
...
In this exercise we're working on a new entry point function called `mint_passport`. Before minting tokens we'll start by implementing some guards and restrictions. Owners can hold no more than one passport at a time. The `QueryMsg` has been provided, you'll need to handle things from there.

1. Create a variable called `query_req` and assign it to `QueryRequest::Wasm`. The function argument send it will be a `WasmQuery::Smart` enum, whose members (`contract_addr` and `msg`) can written on their own line.
2. Regarding `WasmQuery`'s first member: assign `contract_addr` the passport contract's address (accessed from `config.passport_contract`), but we'll need it again later so you'll have to [clone](https://doc.rust-lang.org/std/clone/trait.Clone.html) it and use [into](https://doc.rust-lang.org/std/convert/trait.Into.html) to get the right type.
...
4. The final line of the `query_req` declaration will close both the `WasmQuery` enum and the `QueryRequest`.
