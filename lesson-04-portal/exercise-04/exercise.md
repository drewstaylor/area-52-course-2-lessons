...
Since `Metadata` and `Extension` are publicly exported by the `passport-token` library, we can import those types into Portal and make use of them in our `mint_handler`.
